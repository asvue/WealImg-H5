<template>
  <div id='movielistTop250' v-infinite-scroll="loadMores" infinite-scroll-disabled="loading" infinite-scroll-distance="10">

    <div v-for='(movie,idx) in top250Data' :key='idx' @click="getMovieDetail(movie.id)">
...
export default {
 ...
 getMovieDetail(id){
    ...
    loadMore: function() {
      ...
      this.busy = true;
      return axios.get(`${API.host}/movie/top250/${vm.start == '0' ? vm.start : vm.start * 10}/${vm.count}`, {

      })
        .then(function(response) {
          response.data.subjects.forEach(movie => {

            vm.top250Data.push(movie);
          })
          this.busy = false;
          this.start++;
          this.loading = false;
