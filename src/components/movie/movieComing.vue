<template>
  <div id='movielistComing' v-infinite-scroll="loadMores" infinite-scroll-disabled="loading" infinite-scroll-distance="10">

    <div v-for='(movie,idx) in comingData' :key='idx' @click="getMovieDetail(movie.id)">
...
export default {
  ...
  methods: {
    ...
    loadMore: function() {
      ...
      this.busy = true;
      return axios.get(`${API.host}/movie/coming_soon/${vm.start == '0' ? vm.start : vm.start * 10}/${vm.count}`, {

      })
        .then(function(response) {
          response.data.subjects.forEach(movie => {

            vm.comingData.push(movie);
          })
          this.busy = false;
          this.start++;
          this.loading = false;
